...
const CACHE_KEY = 'proyectos_ciclos_cache';
...
export function useProyectosCiclos() {
  ...
  const getValidCache = useCallback(() => {
    try {
      ...
      const now = Date.now();
      const cacheAge = now - Number.parseInt(timestamp, 10);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API}/v1/graficas/proyectos_075/proyectos_por_ciclo_asignacion`, {
        method: 'POST',
        headers: {
      ...
      if (!jsonData.ciclo_1 || !jsonData.ciclo_2) {
